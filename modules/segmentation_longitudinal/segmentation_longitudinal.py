...
class Segmentation(l.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        return {'loss': loss, 'y_true': y_true.detach(), 'y_pred': y_pred.detach(), 'y_true_metric': y_true_metric.detach()}
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        # caluclate loss
        y_pred = self.softmax(y_pred)
        val_loss = self.diceloss(y_pred, y_true)
    ...
    def validation_step_end(self, val_set_output):
    ...
    def on_validation_epoch_end(self, val_epoch_output):
    ...
    def on_test_epoch_end(self, test_epoch_output):
        volume_means, volume_stds, dice_per_patient = get_eval_means_per_patient(self.dice_per_volume)
        mean_dice = np.mean(list(volume_means.values()))
